...
wss.on("connection", function (ws) {
  ws.on("message", function (message) {
    ...
    try {
      console.log("before parsing data ==> ", typeof message);
    ...
    switch (data.type) {
      ...
      case "record":
        blob = data.blob;
        const oldFilePath =
          "./uploads/oldRecordingFile" + new Date().getTime() + ".webm";
        const newFilePath =
          "./uploads/newRecordingFile" + new Date().getTime() + ".mp4";
        fs.writeFile(oldFilePath, Buffer.from(blob), (err) => {
          if (err)
            console.error("something went wrong when saving file !! ", err);
          console.log("File saved !!");
          ffmpeg(oldFilePath)
            .output(newFilePath)
            .videoCodec("libx264") // Set the video codec to H.264
            .audioCodec("aac") // Set the audio codec to AAC
            .videoBitrate("2000k") // Set the video bitrate to 2000 Kbps
            .audioBitrate("128k") // Set the audio bitrate to 128 Kbps
            .size("1280x720") // Set the output video resolution to 1280x720
            .outputFPS(30)
            .on("end", () => {
